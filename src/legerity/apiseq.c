    ...
    VpProfilePtrType pSeqData)
{
    ...
    switch (pSeqData[0] & VP_SEQ_SUBTYPE_MASK) {
        case VP_SEQ_SUBCMD_SIGGEN:
            VP_DOUT(DBG_SEQUENCER, ("Generator Control 0x%02X 0x%02X",
        ...
        case VP_SEQ_SUBCMD_LINE_STATE:
            VP_DOUT(DBG_SEQUENCER, ("Line State Control 0x%02X 0x%02X",
                pSeqData[0], pSeqData[1]));
        ...
        case VP_SEQ_SUBCMD_WAIT_ON:
            if (pLineObj->pCidProfileType1 != VP_PTABLE_NULL) {
                ...
                if ((pSeqData[0] & VP_SEQ_SUBTYPE_MASK) == VP_SEQ_SUBCMD_WAIT_ON) {
                    VP_DOUT(DBG_CID, ("880 Command WAIT_ON_CID"));
                    pLineObj->callerId.status |= VP_CID_WAIT_ON_ACTIVE;
                } else {
                    VP_DOUT(DBG_CID, ("880 Command START_CID"));
                }
        ...
        case VP_SEQ_SUBCMD_RAMP_GENERATORS:
            VP_DOUT(DBG_SEQUENCER, ("Ramp Generators Control 0x%02X 0x%02X",
                pSeqData[0], pSeqData[1]));
        ...
        case VP_SEQ_SUBCMD_METERING:
            VP_DOUT(DBG_SEQUENCER, ("Metering Control 0x%02X 0x%02X",
                pSeqData[0], pSeqData[1]));
    ...
    if (pLineObj->cadence.index <=
       (pLineObj->cadence.length + VP_PROFILE_LENGTH + 1)) {
        ...
        VP_DOUT(DBG_SEQUENCER, ("Ending Cadence Length %d at Index %d",
    ...
    bool mode)                  /**< TRUE = enabled, FALSE = disable tones */
{
    ...
    VP_DOUT(DBG_CID, ("Writing 0x%02X to SignGen Ctrl", sigGenCtrl));
    ...
    uint8 data)                 /**< 8-bit message data */
{
    ...
    VP_DOUT(DBG_CID, ("Writing 0x%02X to CID Params", fskParam[0]));
    VpMpiCmd(deviceId, ecVal[channelId], VP880_CID_PARAM_WRT,
        VP880_CID_PARAM_LEN, fskParam);

    if (moreData == TRUE) {
        VP_DOUT(DBG_CID, ("Writing 0x%02X to CID Data", data));
    ...
    void *pStruct)
{
    ...
    switch(type) {
        ...
        case VP_SENDSIG_TIP_OPEN_PULSE:
            if (pStruct != VP_NULL) {
                VP_DOUT(DBG_SEQUENCER, ("Pulse Time %d", *((uint16 *)pStruct)));
