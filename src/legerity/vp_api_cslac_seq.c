                                 ...
                                 */
{
    ...
    VP_DOUT(DBG_SEQUENCER, ("Sequence Command 0x%02X 0x%02X",
    ...
    VpProfilePtrType pSeqData)
{
...
#if defined (VP_CC_890_SERIES)
        case VP_DEV_890_SERIES:
            if (!(((Vp890DeviceObjectType *)pDevObj)->status.state & VP_DEV_INIT_CMP)) {
                return VP_STATUS_DEV_NOT_INITIALIZED;
            }
             ...
             * damage the device.
             */
            if (((Vp890DeviceObjectType *)pDevObj)->status.state & VP_DEV_IN_CAL) {
                return VP_STATUS_DEV_NOT_INITIALIZED;
            }
    ...
    if (pCadence->status & VP_CADENCE_STATUS_BRANCHING) {
        VP_DOUT(DBG_SEQUENCER, ("Branching Length %d Index %d At %d",
        ...
        VP_DOUT(DBG_SEQUENCER, ("Branch To %d, Count %d",
    ...
    VpProfilePtrType pSeqData)
{
    ...
    if (pCadence->status & VP_CADENCE_STATUS_MID_TIMER) {
        ...
        VP_DOUT(DBG_SEQUENCER, ("Time Operator %d", pCadence->timeRemain));
    ...
    if (pCadence->timeRemain == 0) {
        ...
        VP_DOUT(DBG_SEQUENCER, ("1. Time Operator Increment Index to %d",
            pCadence->index));

        if (pCadence->index < (pCadence->length + VP_PROFILE_LENGTH + 1)) {
            VP_DOUT(DBG_SEQUENCER, ("Time Operator -- Current Data 0x%02X 0x%02X pCad 0x%02X 0x%02X",
                pSeqData[0], pSeqData[1], pCadence->pCurrentPos[0], pCadence->pCurrentPos[1]));
            pSeqData+=2;
            pCadence->pCurrentPos = pSeqData;
            VP_DOUT(DBG_SEQUENCER, ("Time Operator -- Next Data 0x%02X 0x%02X pCad 0x%02X 0x%02X",
                pSeqData[0], pSeqData[1], pCadence->pCurrentPos[0], pCadence->pCurrentPos[1]));
        ...
        if ((pCid != VP_NULL) && (pCid->status & VP_CID_WAIT_ON_ACTIVE)) {
            VP_DOUT(DBG_CID, ("CID Status 0x%04X", pCid->status));
            if (pCid->status & VP_CID_IN_PROGRESS) {
                /* Do nothing */
                VP_DOUT(DBG_CID, ("CID In Progress"));
            } else {
                VP_DOUT(DBG_CID, ("Terminating Timer"));

                /* Terminate this timer operation and the wait on */
                pCid->status &= ~ VP_CID_WAIT_ON_ACTIVE;
                pCadence->status &= ~VP_CADENCE_STATUS_MID_TIMER;
                pCadence->timeRemain = 0;
                VP_DOUT(DBG_SEQUENCER, ("2. Time Operator Increment Index to %d",
                    pCadence->index));
    ...
    VpLineCtxType *pLineCtx)    /**< Line that has an active CID sequence */
{
    ...
    switch (deviceType) {
        ...
        case VP_DEV_890_SERIES:
            if (!(((Vp890LineObjectType *)pLineObj)->status & VP890_IS_FXO)) {
                ...
                SetLineState = Vp890SetFxoLineState;
            }

            pCidStruct = &((Vp890LineObjectType *)pLineObj)->callerId;
            if (!(((Vp890DeviceObjectType *)pDevObj)->status.state & VP_DEV_INIT_CMP)) {
                VP_DOUT(DBG_CID, ("1. VpCidSeq() -- VP_STATUS_DEV_NOT_INITIALIZED"));
             ...
             */
            if (((Vp890DeviceObjectType *)pDevObj)->status.state & VP_DEV_IN_CAL) {
                VP_DOUT(DBG_CID, ("2. VpCidSeq() -- VP_STATUS_DEV_NOT_INITIALIZED"));
    ...
    if(pCidStruct->cliTimer > 0) {
        ...
        if (pCidStruct->status & VP_CID_REPEAT_MSG) {
            while (VpFSKGeneratorReady(pLineCtx) == TRUE) {
        ...
        if(pCidStruct->cliTimer != 0) {
            VP_DOUT(DBG_CID, ("1. VpCidSeq() -- Running Timer %d",
                pCidStruct->cliTimer));
            return VP_STATUS_SUCCESS;
        }
    } else {
        VP_DOUT(DBG_CID, ("2. VpCidSeq() -- Running Timer %d",
            pCidStruct->cliTimer));
        return VP_STATUS_SUCCESS;
    }
    ...
    if (pCidStruct->status & VP_CID_AWAIT_TONE) {
        ...
        if ((digit == pCidStruct->cliDetectTone1)
         || (digit == pCidStruct->cliDetectTone2)) {
            ...
            VP_DOUT(DBG_CID, ("Ack Tone Not Detected"));
            return VP_STATUS_SUCCESS;
        }
    }
    ...
    if (pCidStruct->status & VP_CID_TERM_FSK) {
        ...
        VP_DOUT(DBG_CID, ("Terminating FSK"));
    ...
    VP_DOUT(DBG_CID, ("CID Op-Code 0x%02X", uiCliOpCode));
    ...
    while ( (index <= pCidStruct->pCliProfile[startOfCliData - 2]) &&
        ...
        switch (uiCliOpCode) {
            ...
            case VP_CLI_MUTEON:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_MUTEON"));
                index+=2;
             ...
             */
            case VP_CLI_MUTEOFF:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_MUTEOFF"));
                index+=2;
                pCidStruct->status &= ~(VP_CID_MUTE_ON);
                VpCtrlMuteChannel(pLineCtx, FALSE);
                break;
            ...
            case VP_CLI_POLREV:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_POLREV"));
            ...
            case VP_CLI_EOT:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_EOT"));
                VpCliStopCli(pLineCtx);
                return VP_STATUS_SUCCESS;
    ...
    if (index <= pCidStruct->pCliProfile[startOfCliData - 2]) {
        ...
        switch (pCidStruct->pCliProfile[startOfCliData + index]) {
             ...
             */
            case VP_CLI_DETECT:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_DETECT"));
            ...
            case VP_CLI_ALERTTONE:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_ALERTTONE"));
            ...
            case VP_CLI_ALERTTONE2:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_ALERTTONE2"));
             ...
             */
            case VP_CLI_SILENCE:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_SILENCE"));
            ...
            case VP_CLI_SILENCE_MASKHOOK:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_SILENCE_MASKHOOK"));
             ...
             */
            case VP_CLI_CHANSEIZURE:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_CHANSEIZURE"));
              ...
              */
            case VP_CLI_MARKSIGNAL:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_MARKSIGNAL"));
            ...
            case VP_CLI_MESSAGE:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_MESSAGE"));
            ...
            case VP_CLI_DTMF_MESSAGE:
                VP_DOUT(DBG_CID, ("CID Op-Code VP_CLI_DTMF_MESSAGE"));
            ...
            default:
                VP_DOUT(DBG_CID, ("CID Op-Code ERROR"));
