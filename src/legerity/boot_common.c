 ...
 * is applicable to VCP and VPP classes of devices.
 *
...
/**
 * VpVcpVppBootLoad()
 *  Loads an executable image into the VTD (VCP/VPP).
 *
    ...
    )
{
    ...
    /* Pass VCP/VPP specific halt, reset, and validate functions to
     * VpBootLoadInternal(). */
    return VpBootLoadInternal(state, pImageBuffer, bufferSize, pScratchMem,
        validation, deviceId, pRecCnt, VpHbiVcpVppClearCodeMem, VpHbiVcpVppHalt,
        VpHbiVcpVppValidate);
}
...
VpBootLoadInternal(
    ...
    VpDeviceIdType deviceId,
    uint16p pRecCnt,
    VpBootSupportFuncPtrType resetFunc,
        ...
        while (newblocks--) {
            ...
            if (!veCompressTestEnable) {
                ...
                *pRecCnt = ((uint16)pHbiBlock[6] << 8) + pHbiBlock[7];
                ...
                /* Initialize the HBI and reset the VTD. */
                if (
                    (VpHalHbiInit(deviceId) == FALSE) ||
                    (resetFunc(deviceId) == FALSE) /* initiate HBI boot sequence */
                ) {
                    VP_ERROR(VpDeviceIdType, &deviceId, ("VpBootLoadInternal(): Failed VpHalHbiInit or resetFunc"));
...
#ifdef VP_COMPRESS_TEST
        if (veCompressTestEnable) {
            stream->adler = adler32(stream->adler, pHbiBlock, 128);
        } else {
#endif
